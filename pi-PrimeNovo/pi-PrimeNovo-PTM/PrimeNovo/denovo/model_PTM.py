...
#true model for PrimeNovo, for ft PTM + class head
    ...
    **kwargs : Dict
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        self.peptide_mass_calculator = depthcharge.masses.PeptideMass_PTM(
            self.residues)
    ...
    def on_load_checkpoint(self, checkpoint: dict) -> None:
        ...
        state_dict_loaded = checkpoint["state_dict"]
        state_dict_intialized = self.state_dict()
        #is_changed = False
        for k in state_dict_intialized:
            if 28 in list(state_dict_loaded[k].shape):
                    state_dict_loaded[k] = state_dict_intialized[k]
    def _forward_step(
        self,
    ...
    def validation_step(self, batch, batch_idx=None, dataloader_idx=None) -> torch.Tensor:
        ...
        if(self.n_beams>0):
            ...
            true_peps = batch[2]
            head_value = self.class_head(last_logits)
            head_value = head_value = head_value[:, -1, :] #bz, 2
            predictions_class = torch.argmax(head_value, dim=1)# [bz]
            predictions_class = predictions_class.tolist()
            result_label = [0 for each in range(pred.shape[0])]
            True_positive = 0
            False_positive = 0
            True_negative = 0
            ...
            for idx, each in enumerate(true_peps):
            #print(each)
                if "B" in each and predictions_class[idx] == 1:
                    #print("found PTM!")
                    #result_label[idx] = 1
                    True_positive +=1
                elif "B" in each and predictions_class[idx] == 0:
                    False_negative+=1
                elif "B" not in each and predictions_class[idx] == 0:
                    True_negative +=1 
                else:
                    False_positive +=1
            log_args = dict(on_step=True, on_epoch=True, sync_dist=True, add_dataloader_idx=False)
            ...
            self.log("{}/pred_acc".format(key), (True_positive + True_negative) / len(result_label), **log_args)
            ...
            for peptide_pred, peptide_true in zip(peptides_pred_raw, batch[2]):
                ...
                if "B" in peptide_pred and "B" in peptide_true:
                    True_positive +=1 
                if "B" not in peptide_pred and "B" not in peptide_true:
                    True_negative +=1
            log_args = dict(on_step=True, on_epoch=True, sync_dist=True, add_dataloader_idx=False)
            # self.log("{}/spec/second".format(key), calctime, **log_args)
            # self.log("{}/M+0.984_precision".format(key), AA_sim_true["M+15.995"] / AA_sim_sum["M+15.995"], **log_args)
            ...
            self.log("{}/pred_acc2".format(key), (True_positive + True_negative) / len(result_label), **log_args)
            ...
            #     with open("./score_Kmod_Biotinyl.txt",'a') as f:
            #         sum = np.sum(matches)
            #         aaPre = sum / len(pred)
            ...
            sys.stdout.flush()
            with open("./denovo_luad_protein_top2.txt",'a') as f:
                for i in range(len(peptides_pred_raw)):
                    ...
                    f.write("label: " + batch[2][i] + " prediction: " + "".join(peptides_pred_raw[i]) + " charge: " + str(int(batch[1][i][1])) + " score: " + str(float(inferscores[i]))+"\n")
    ...
    def predict_step(
        ...
        with open("./denovo_laud_PMC.txt",'a') as f:
